    ...
    assignNames(names, forbiddenNamesIterable, namesToAssignIterable) {
        ...
        for (const name of namesToAssign) {
            ...
            if (maybeUniqueName !== undefined) {
                ...
                var prefix = '';
                if (prependSubTypeWithRoot) {
                    var iterProposedNames = proposedNames.values();
                    const proposedNameFirst = iterProposedNames.next();
                    const proposedNameSecond = iterProposedNames.next();
                    if (proposedNameSecond.value != undefined && proposedNameSecond.value.indexOf('_class') >= 0) {
                        prefix = subTypePrefix;
                    }

                }

                const styledName = namingFunction.nameStyle(prefix + '_' + maybeUniqueName);
        ...
        for (; ;) {
            ...
            for (const [namer, namedsForNamingFunction] of byNamingFunction) {
                ...
                for (const [, nameds] of byProposed) {
                    ...
                    const names = namer.assignNames(ctx.names, forbiddenNames, nameds, prependSubTypeWithRoot, subTypePrefix);
